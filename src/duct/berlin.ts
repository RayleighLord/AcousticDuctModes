 ...
 * map designed for dark backgrounds. Its blue and coral ends show negative and
 * positive pressure while its dark centre marks zero pressure.
 *
 * Source: https://doi.org/10.5281/zenodo.8409685
 * Copyright (c) 2023 Fabio Crameri, distributed under the MIT licence.
 */
const BERLIN_HEX =
...
/** Create the one-dimensional sRGB lookup texture used by the pressure shader. */
...
/** Map signed normalized pressure from [-1, 1] to the Berlin LUT domain. */
export function berlinCoordinate(pressure: number): number {
  return (THREE.MathUtils.clamp(pressure, -1, 1) + 1) * 0.5;
}
